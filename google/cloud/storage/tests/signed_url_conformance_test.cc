...
TEST_P(V4PostPolicyConformanceTest, V4PostPolicy) {
  ...
  ASSERT_TRUE(condition.startswith().size() == 0 ||
              condition.startswith().size() == 2);
  ...
  ASSERT_TRUE(condition.contentlengthrange().size() == 0 ||
              condition.contentlengthrange().size() == 2);
...
int main(int argc, char* argv[]) {  // NOLINT(bugprone-exception-escape)
  ...
  std::ifstream ifstr(conformance_tests_file, std::ios_base::binary);
  if (!ifstr.is_open()) {
  ...
  auto status =
      google::protobuf::util::JsonStringToMessage(buffer.str(), &tests);
  if (!status.ok()) {
