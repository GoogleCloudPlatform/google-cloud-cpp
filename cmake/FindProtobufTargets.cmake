...
Using ``find_package(protobuf)`` should be simple, but unfortunately it is not.
...
On some Linux distributions (e.g. Fedora>=29, and openSUSE-Tumbleweed) the are
system packages for protobuf, but these packages are installed without the CMake
support files, and one must either use the ``FindProtobuf`` module, find the
libraries via `pkg-config`, or find the libraries manually.

When compiling with `vcpkg` the CMake config files (i.e.
``protobuf-config.cmake``) *are* installed. These config files produce the same
targets as recent versions of ``FindProtobuf`` does, but they do not produce
the ``Protobuf_LIBRARY``, ``Protobuf_INCLUDE_DIR``, etc. that are generated by
the module.

This module is designed to provide a single, uniform, `find_package()` interface
that works for all cases described above. It works as follows:
...
# Note that on Linux this *must* be all lowercase ``protobuf``, while on Windows
# it does not matter.
...
if (protobuf_FOUND)
    ...
    if (Protobuf_FOUND)
        ...
        if (NOT TARGET protobuf::libprotobuf)
            ...
            set_property(TARGET protobuf::libprotobuf
                APPEND
                PROPERTY INTERFACE_LINK_LIBRARIES ${Protobuf_LIBRARY} Threads::Threads)
        endif ()
        ...
        if (NOT TARGET protobuf::libprotobuf-lite)
            ...
            set_property(TARGET protobuf::libprotobuf-lite
                APPEND
                PROPERTY INTERFACE_LINK_LIBRARIES ${Protobuf_LITE_LIBRARY} Threads::Threads)
        endif ()
        ...
        if (NOT TARGET protobuf::libprotoc)
            ...
            set_property(TARGET protobuf::libprotoc
                    APPEND
                    PROPERTY INTERFACE_LINK_LIBRARIES ${Protobuf_PROTOC_LIBRARY} Threads::Threads)
        endif ()
    endif ()
endif ()
...
if (ProtobufTargets_FOUND AND NOT TARGET protobuf::protoc)
    ...
    set_property(TARGET protobuf::protoc
            PROPERTY IMPORTED_LOCATION_RELEASE ${_protobuf_PROTOC_EXECUTABLE})
...
if (protobuf_DEBUG)
    ...
    if (ProtobufTargets_FOUND)
        foreach (_target protobuf::libprotobuf protobuf::libprotobuf-lite protobuf::libprotoc)
