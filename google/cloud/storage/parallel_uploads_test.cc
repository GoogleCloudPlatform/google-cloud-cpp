...
TEST(PersistentState, NotJson) {
  auto res = PersistentState::FromString("blah");
  EXPECT_FALSE(res);
  EXPECT_EQ(StatusCode::kInternal, res.status().code());
  EXPECT_THAT(res.status().message(), HasSubstr("not a valid JSON"));
}

TEST(PersistentState, RootNotOject) {
  auto res = PersistentState::FromString("\"blah\"");
  EXPECT_FALSE(res);
  EXPECT_EQ(StatusCode::kInternal, res.status().code());
...
TEST(PersistentState, NoDestination) {
  auto res = PersistentState::FromString(internal::nl::json{{"a", "b"}}.dump());
  EXPECT_FALSE(res);
  EXPECT_EQ(StatusCode::kInternal, res.status().code());
...
TEST(PersistentState, DestinationNotAString) {
  auto res = PersistentState::FromString(
...
TEST(PersistentState, NoGeneration) {
  auto res = PersistentState::FromString(
...
TEST(PersistentState, GenerationNotAString) {
  auto res = PersistentState::FromString(internal::nl::json{
...
TEST(PersistentState, NoStreams) {
  auto res = PersistentState::FromString(
...
TEST(PersistentState, StreamsNotArray) {
  auto res = PersistentState::FromString(internal::nl::json{
...
TEST(PersistentState, StreamNotObject) {
  auto res = PersistentState::FromString(internal::nl::json{
      {"destination", "dest"}, {"expected_generation", 1}, {"streams", {5}}}
                                             .dump());
  EXPECT_FALSE(res);
...
TEST(PersistentState, StreamHasNoName) {
  auto res = PersistentState::FromString(
...
TEST(PersistentState, StreamNameNotString) {
  auto res = PersistentState::FromString(
      internal::nl::json{{"destination", "dest"},
                         {"expected_generation", 1},
                         {"streams", {{{"name", 1}}}}}
...
TEST(PersistentState, StreamHasNoSessionId) {
  auto res = PersistentState::FromString(
      internal::nl::json{{"destination", "dest"},
                         {"expected_generation", 1},
                         {"streams", {{{"name", "abc"}}}}}
...
TEST(PersistentState, StreamSessionIdNotString) {
  auto res = PersistentState::FromString(internal::nl::json{
      {"destination", "dest"},
      {"expected_generation", 1},
...
TEST(PersistentState, StreamsEmpty) {
  auto res = PersistentState::FromString(
      internal::nl::json{{"destination", "dest"},
                         {"expected_generation", 1},
