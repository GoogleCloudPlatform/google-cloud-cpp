...
#include "google/cloud/storage/grpc_resumable_upload_session_url.pb.h"
...
StatusOr<ResumableUploadSessionGrpcParams> DecodeGrpcResumableUploadSessionUrl(
  ...
  std::string const payload =
      upload_session_url.substr(std::strlen(kURIScheme));
  ...
  if (!proto.ParseFromString(decoded)) {
    return Status(StatusCode::kInvalidArgument,
                  "Malformed gRPC resumable uplpad session URL");
  }
...
bool IsGrpcResumableSessionUrl(std::string const& upload_session_url) {
  return upload_session_url.find(kURIScheme) == 0;
}
