...
int main(int argc, char* argv[]) {
  ...
  // them so far.
  std::set<std::string> nonconformant_url_tests{
      "ListObjects", "HTTPBucketBoundHostnameSupport",
      "HTTPSBucketBoundHostnameSupport"};
  ...
  for (auto const& j_obj : signing_tests_json.items()) {
    ...
    bool inserted =
        google::cloud::storage::signing_tests->emplace(name, j_obj.value())
            .second;
    if (!inserted) {
  ...
  // The implementation is not yet completed and these tests still fail, so skip
  // them so far.
  std::set<std::string> nonconformant_post_tests{"POSTPolicyCharacterEscaping"};
  ...
  for (auto const& j_obj : post_policy_tests_json.items()) {
    ...
    if (nonconformant_post_tests.find(name) != nonconformant_post_tests.end()) {
      continue;
    }
    bool inserted =
