...
TEST_P(V4SignedUrlConformanceTest, V4SignJson) {
  ...
  DomainNamedBucket domain_named_bucket;
  if (url_style == "BUCKET_BOUND_DOMAIN") {
    domain_named_bucket = DomainNamedBucket(j_obj["bucketBoundDomain"]);
    request.set_multiple_options(DomainNamedBucket(j_obj["bucketBoundDomain"]));
  }
  ...
  auto actual = client.CreateV4SignedUrl(
      ...
      query_params[4], virtual_hotname, domain_named_bucket);
