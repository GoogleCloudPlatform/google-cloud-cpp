...
using ::google::cloud::internal::IsIdempotent;
 ...
 public:
    ...
    return ListDatabaseRange(
        ...
        [stub, retry, backoff,
         function_name](gcsa::ListDatabasesRequest const& r) {
          return RetryLoop(
              retry->clone(), backoff->clone(), IsIdempotent::kIdempotent,
  ...
  StatusOr<google::iam::v1::Policy> SetIamPolicy(
    ...
    *request.mutable_policy() = std::move(p.policy);
    IsIdempotent is_idempotent = request.policy().etag().empty()
                                     ? IsIdempotent::kNonIdempotent
                                     : IsIdempotent::kIdempotent;
    return RetryLoop(
        retry_policy_prototype_->clone(), backoff_policy_prototype_->clone(),
